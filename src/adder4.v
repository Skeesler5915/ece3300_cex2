...
wire carry[2:0];
	fulladd addc0 
	(.x(a[0]), .y(b[0]), .cin(1'b0),.sum(result[0]),.cout(carry[0]));
	fulladd addc1 
	(.x(a[1]), .y(b[1]), .cin(cout0),.sum(result[1]),.cout(carry[1]));
	fulladd addc2 
	(.x(a[2]), .y(b[2]), .cin(cout1),.sum(result[2]),.cout(carry[2]));
	fulladd addc3 
	(.x(a[3]), .y(b[3]), .cin(cout2),.sum(result[3]),.cout(result[4]));
